...
from quiz.pipeline import DocumentParser


# --- DocumentParser._clean ---

def test_clean_text_replaces_fi_ligature():
    assert DocumentParser._clean("ﬁle") == "file"


def test_clean_text_replaces_fl_ligature():
    assert DocumentParser._clean("ﬂoor") == "floor"


def test_clean_text_replaces_ff_ligature():
    assert DocumentParser._clean("diﬀerence") == "difference"


def test_clean_text_removes_null_bytes():
    assert "\x00" not in DocumentParser._clean("hel\x00lo")


def test_clean_text_removes_control_characters():
    assert "\x08" not in DocumentParser._clean("back\x08space")


def test_clean_text_removes_standalone_page_numbers():
    result = DocumentParser._clean("Introduction\n42\nConclusion")
    assert "42" not in result.split()


def test_clean_text_does_not_remove_inline_numbers():
    result = DocumentParser._clean("There are 42 species in this genus.")
    assert "42" in result


def test_clean_text_collapses_excessive_newlines():
    result = DocumentParser._clean("line1\n\n\n\n\nline2")
    assert "\n\n\n" not in result


def test_clean_text_preserves_normal_sentence():
    text = "The mitochondria is the powerhouse of the cell."
    assert DocumentParser._clean(text) == text


def test_clean_text_strips_leading_and_trailing_whitespace():
    assert DocumentParser._clean("  hello world  ") == "hello world"


# --- DocumentParser.chunk ---

def test_chunk_text_empty_returns_empty_list():
    assert DocumentParser().chunk("") == []


def test_chunk_text_short_text_produces_single_chunk():
    text = "short text with just a few words"
    chunks = DocumentParser(chunk_size=500, overlap=50).chunk(text)
    assert len(chunks) == 1
...
def test_chunk_text_produces_correct_count():
    ...
    chunks = DocumentParser(chunk_size=50, overlap=10).chunk(words)
    assert len(chunks) == 3


def test_chunk_text_each_chunk_respects_max_size():
    words = " ".join(f"word{i}" for i in range(1000))
    chunks = DocumentParser(chunk_size=100, overlap=10).chunk(words)
    for chunk in chunks:
...
def test_chunk_text_overlap_between_consecutive_chunks():
    ...
    chunks = DocumentParser(chunk_size=100, overlap=20).chunk(" ".join(words))
...
def test_chunk_text_default_params_produce_multiple_chunks():
    ...
    chunks = DocumentParser().chunk(words)
    assert len(chunks) == 2
...
def test_extract_text_txt_returns_content():
    ...
    assert "Hello world" in DocumentParser().extract(mock_file)
...
def test_extract_text_txt_applies_clean_text():
    ...
    result = DocumentParser().extract(mock_file)
    assert "fi" in result
...
def test_extract_text_unsupported_format_raises():
    ...
    with pytest.raises(ValueError, match="Unsupported file type"):
        DocumentParser().extract(mock_file)
