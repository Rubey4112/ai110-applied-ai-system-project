...
def test_send_message_appends_transcript_and_titles_session(monkeypatch):
    ...
    box, history, sessions, choices_update = main.send_message(
...
def test_send_message_ignores_blank_input(monkeypatch):
    ...
    box, history, sessions, _ = main.send_message("   ", session_id, sessions, order)
...
def test_new_chat_and_switch_session_round_trip(monkeypatch):
    ...
    _, history, sessions, _ = main.send_message("hype workout mix", first_id, sessions, order)
    assert sessions[first_id]["messages"] == history

    second_id, sessions, order, chatbot_reset, choices_update = main.new_chat(sessions, order)

    assert second_id != first_id
    assert chatbot_reset == []
    assert order == [second_id, first_id]

    restored_id, restored_history = main.switch_session(first_id, sessions)
